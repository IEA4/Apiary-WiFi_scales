...
ПЕРСПЕКТИВУ БУДУЩИХ ИЗМЕНЕНИЙ), А ТАКЖЕ ФАЙЛА С МАССИВОМ ИЗМЕРЕНИЙ И С
СООТВЕТСТВУЮЩИХ ИМ unix-ВРЕМЕН ИЗМЕРЕНИЯ (ИСТОРИЯ ПОКАЗАНИЙ)

ЗАГРУЗИТЬ СКЕТЧ, ПЕРЕЗАГРУЗИТЬ, ЗАЛИТЬ ФАЙЛ В ФАЙЛОВУЮ СИСТЕМУ ESP8266. ПИН
GPIO16 СОЕДИНИТЬ С ПИНОМ RST.

ДЛИТЕЛЬНОСТЬ СНА НЕ БОЛЕЕ 2 ЧАСОВ. МАКСИМАЛЬНО ВОЗМОЖНАЯ ДЛИТЕЛЬНОСТЬ НЕМНОГИМ
БОЛЬШЕ ЭТОГО ВРЕМЕНИ, ПЛЮС ОНО НЕ ПОСТОЯННО

НЕОБХОДИМО ПРОПИСАТЬ СВОИ НАСТРОЙКИ WI-FI
*/
